...
func TestActivityRepo(t *testing.T) {
	...
	if _, err := repo.Create(ctx, tx, []*types.Activity{a1, a2, a3}); err != nil {
	...
	if rows, err := repo.GetByIDs(ctx, tx, []uuid.UUID{a1.ID, a2.ID, a3.ID}); err != nil || len(rows) != 3 {
		t.Fatalf("GetByIDs: err=%v len=%d", err, len(rows))
	}
	if rows, err := repo.ListByOwner(ctx, tx, ownerType, testutil.PtrUUID(owner1)); err != nil || len(rows) != 2 {
		t.Fatalf("ListByOwner: err=%v len=%d", err, len(rows))
	}
	if rows, err := repo.ListByOwnerIDs(ctx, tx, ownerType, []uuid.UUID{owner1, owner2}); err != nil || len(rows) != 3 {
		t.Fatalf("ListByOwnerIDs: err=%v len=%d", err, len(rows))
	}
	if rows, err := repo.ListByStatus(ctx, tx, []string{"ready"}); err != nil || len(rows) != 1 {
	...
	if err := repo.UpdateFields(ctx, tx, a2.ID, map[string]interface{}{"status": "archived"}); err != nil {
		t.Fatalf("UpdateFields: %v", err)
	}

	if err := repo.SoftDeleteByOwner(ctx, tx, ownerType, testutil.PtrUUID(owner1)); err != nil {
		t.Fatalf("SoftDeleteByOwner: %v", err)
	}
	if rows, err := repo.ListByOwner(ctx, tx, ownerType, testutil.PtrUUID(owner1)); err != nil || len(rows) != 0 {
	...
	if _, err := repo.Create(ctx, tx, []*types.Activity{b1}); err != nil {
	...
	if _, err := repo.Create(ctx, tx, []*types.Activity{b2}); err != nil {
		t.Fatalf("seed b2: %v", err)
	}
	if err := repo.FullDeleteByOwner(ctx, tx, ownerType, testutil.PtrUUID(owner2)); err != nil {
