...
import (
	...
	"github.com/yungbote/neurobridge-backend/internal/platform/normalize"
...
func InputValidation(ctx context.Context, ffor string, userRepo repos.UserRepo, log *logger.Logger, user *types.User, email, password string) error {
	validatedFor := normalize.ParseInputString(ffor)
	if validatedFor == "" {
...
func NormalizeUserFields(ctx context.Context, user *types.User) {
	user.Email = normalize.ParseInputString(user.Email)
	user.Password = normalize.ParseInputString(user.Password)
	user.FirstName = normalize.ParseInputString(user.FirstName)
	user.LastName = normalize.ParseInputString(user.LastName)
}
