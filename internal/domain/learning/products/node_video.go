...
// LearningNodeVideo stores a planned (and optionally rendered) short video for a PathNode.
// Video planning and rendering are separated from NodeDoc generation so NodeDoc only references
// already-available assets.
type LearningNodeVideo struct {
	ID uuid.UUID `gorm:"type:uuid;default:uuid_generate_v4();primaryKey" json:"id"`

	UserID     uuid.UUID `gorm:"type:uuid;not null;index:idx_learning_node_video_key,unique,priority:1;index" json:"user_id"`
	PathID     uuid.UUID `gorm:"type:uuid;not null;index" json:"path_id"`
	PathNodeID uuid.UUID `gorm:"type:uuid;not null;index:idx_learning_node_video_key,unique,priority:2;index" json:"path_node_id"`
	Slot       int       `gorm:"column:slot;not null;index:idx_learning_node_video_key,unique,priority:3" json:"slot"`
...
func (LearningNodeVideo) TableName() string { return "learning_node_video" }
