...
func TestStageDepsForPremiumGrouping(t *testing.T) {
	if !containsStageDep(stageDeps["path_intake"], "file_signature_build") {
		t.Fatalf("path_intake should depend on file_signature_build")
	}
	if !containsStageDep(stageDeps["path_grouping_refine"], "path_intake") {
		t.Fatalf("path_grouping_refine should depend on path_intake")
	}
	if !containsStageDep(stageDeps["path_structure_dispatch"], "path_grouping_refine") {
