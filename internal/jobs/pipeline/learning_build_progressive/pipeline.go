...
func (p *Pipeline) Run(jc *jobrt.Context) error {
	...
	if p == nil || p.db == nil || p.log == nil || p.jobs == nil || p.path == nil || p.saga == nil || p.bootstrap == nil {
		jc.Fail("validate", fmt.Errorf("learning_build_progressive: pipeline not configured"))
		return nil
	}
...
func (p *Pipeline) enqueueNodeDocPrefetch(jc *jobrt.Context, setID uuid.UUID, pathID uuid.UUID) {
	...
	if _, _, err := p.jobs.EnqueueNodeDocPrefetchIfNeeded(dbctx.Context{Ctx: jc.Ctx, Tx: jc.DB}, jc.Job.OwnerUserID, pathID, setID, "learning_build_progressive"); err != nil {
