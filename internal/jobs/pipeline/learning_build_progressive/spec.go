...
const learningBuildPipelineEnv = "LEARNING_BUILD_PROGRESSIVE_PIPELINE_YAML"

//go:embed learning_build_progressive.yaml
...
func currentPipelineRuntime(log *logger.Logger) *pipelineRuntime {
	...
	if runtimeErr != nil {
		if log != nil {
			log.Warn("learning_build_progressive: pipeline spec load failed; using fallback", "error", runtimeErr)
		}
		return nil
	}
	return runtimeCache
...
func readLearningBuildSpec() ([]byte, error) {
	...
	return learningBuildSpecFS.ReadFile("learning_build_progressive.yaml")
}
...
func validatePipelineSpec(spec *yamlPipelineSpec) error {
	...
	if strings.TrimSpace(spec.Pipeline) != "learning_build_progressive" {
