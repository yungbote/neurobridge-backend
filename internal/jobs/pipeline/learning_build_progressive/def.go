...
func New(
	...
	if minPoll < minPollFloor {
		if baseLog != nil {
			baseLog.Warn("learning_build_progressive: min poll too low; clamping", "requested_ms", minPoll.Milliseconds(), "floor_ms", minPollFloor.Milliseconds())
		}
	...
	return &Pipeline{
		db:                db,
		log:               baseLog.With("job", "learning_build_progressive"),
		jobs:              jobs,
		path:              path,
		threads:           threads,
		messages:          messages,
...
func (p *Pipeline) Type() string { return "learning_build_progressive" }
