...
func LibraryTaxonomyRefine(ctx context.Context, deps LibraryTaxonomyRouteDeps, in LibraryTaxonomyRefineInput) (LibraryTaxonomyRefineOutput, error) {
	...
	if deps.Graph != nil {
		for _, facet := range defaultTaxonomyFacets {
			if err := syncLibraryTaxonomyFacetToNeo4j(ctx, deps, in.UserID, facet); err != nil && deps.Log != nil {
				deps.Log.Warn("neo4j library taxonomy sync failed (continuing)", "error", err, "user_id", in.UserID.String(), "facet", facet)
			}
		}
	}
	return out, nil
