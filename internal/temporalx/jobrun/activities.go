...
func (a *Activities) Tick(ctx context.Context, jobID string) (TickResult, error) {
	...
	defer func() {
		if a != nil && a.Metrics != nil {
			a.Metrics.ObserveActivity(ActivityTick, jobType, finalStatus, time.Since(start))
		}
	}()
	...
	jobLog := a.Log
	if jobLog != nil {
		jobLog = jobLog.With("job_id", parsedJobID, "job_type", job.JobType)
		if td := ctxutil.GetTraceData(jc.Ctx); td != nil {
			if td.TraceID != "" {
				jobLog = jobLog.With("trace_id", td.TraceID)
			}
			if td.RequestID != "" {
				jobLog = jobLog.With("request_id", td.RequestID)
			}
		}
	}
	if !ok {
		...
		func() {
			defer func() {
				if r := recover(); r != nil {
					if jobLog != nil {
						jobLog.Error("Job handler panic", "panic", r)
					}
	...
	if handlerReturnedNil && strings.EqualFold(strings.TrimSpace(updated.Status), "running") {
		if jobLog != nil {
			jobLog.Warn("Job handler returned nil without terminal status; marking succeeded", "stage", updated.Stage)
		}
		finalStage := "done"
